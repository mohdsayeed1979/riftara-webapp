...
export const metadata: Metadata = { title: 'New Proposal Version' };

function str(v: unknown): string | undefined {
  return v === null || v === undefined ? undefined : String(v);
}

export default async function ProposalVersionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <PageHeader
        breadcrumbs={[{ label: 'Proposals', href: '/leasing/proposals' }, { label: proposal.reference, href: `/leasing/proposals/${id}` }, { label: 'New Version' }]}
        title={`New Version of ${proposal.reference}`}
        subtitle="Creates a new version and supersedes the current one. The previous version is retained."
      />
      <ProposalForm reference={reference} initial={initial} mode="version" proposalId={id} />
    </div>
  );
}
