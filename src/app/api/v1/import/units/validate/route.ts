...
/** POST /api/v1/import/units/validate — a Units-only workbook/CSV against existing properties. */
export async function POST(request: Request) {
  try {
    ...
    const preview = await buildPreview(principal.organizationId, { propertyRows: [], unitRows: unitSheet?.rows ?? [], mode });
