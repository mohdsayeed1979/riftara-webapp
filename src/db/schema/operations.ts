...
export const maintenanceCosts = pgTable(
  'maintenance_costs',
  {
    ...
    /** Itemized materials/spare-parts capture for costType='parts' lines (Phase 19). */
...
/**
 * One row per (schedule, scheduled occurrence) generated by the preventive
 * scheduler. The unique constraint is the actual idempotency guarantee:
 * concurrent/repeated generator runs for the same schedule + due date can
 * only ever produce one work order (Phase 19).
 */
export const preventiveMaintenanceOccurrences = pgTable(
  'preventive_maintenance_occurrences',
  {
    ...
    scheduleId: uuid('schedule_id')
      .notNull()
      .references(() => preventiveMaintenanceSchedules.id, { onDelete: 'cascade' }),
  ...
  (t) => [
    unique('pm_occurrences_schedule_date_uq').on(t.scheduleId, t.occurrenceDate),
    index('pm_occurrences_org_idx').on(t.organizationId),
  ],
);

/** Itemized maintenance checklist/inspection template (Phase 19 — distinct from the Phase 17 handover inspection). */
export const maintenanceChecklistTemplates = pgTable(
  'maintenance_checklist_templates',
  {
    ...
    nameAr: varchar('name_ar', { length: 160 }),
    categoryId: uuid('category_id').references(() => maintenanceCategories.id, { onDelete: 'set null' }),
    /** Ordered item definitions: [{ key, labelEn, labelAr, required, createsCorrectiveOnFail }] */
    items: jsonb('items')
      .$type<Array<{ key: string; labelEn: string; labelAr?: string; required?: boolean; createsCorrectiveOnFail?: boolean }>>()
      .notNull()
      .default([]),
    isActive: boolean('is_active').notNull().default(true),
    isDemo: isDemo(),
  ...
  (t) => [
    unique('maintenance_checklist_templates_org_code_uq').on(t.organizationId, t.code),
    index('maintenance_checklist_templates_org_idx').on(t.organizationId),
  ],
);

/** Per-work-order checklist execution results. Upserted per (workOrder, template, item) — reruns update, never duplicate. */
export const workOrderChecklistResults = pgTable(
  'work_order_checklist_results',
  {
    ...
    templateId: uuid('template_id')
      .notNull()
      .references(() => maintenanceChecklistTemplates.id, { onDelete: 'cascade' }),
    itemKey: varchar('item_key', { length: 64 }).notNull(),
    itemLabelEn: varchar('item_label_en', { length: 200 }).notNull(),
    /** pass | fail | na */
    result: varchar('result', { length: 8 }).notNull(),
    notes: text('notes'),
    documentId: uuid('document_id'),
    correctiveWorkOrderId: uuid('corrective_work_order_id').references(() => workOrders.id, { onDelete: 'set null' }),
  ...
  (t) => [
    unique('wo_checklist_results_item_uq').on(t.workOrderId, t.templateId, t.itemKey),
    index('wo_checklist_results_wo_idx').on(t.workOrderId),
  ],
);
